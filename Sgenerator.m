...
%STEP 3
for i = 2:(2*n-1)
    if(mod(i,2)==1) %opposite conditions to paper, matlab uses 1 indexing
        %add 1 to the row
        row = R(i-1);
        temprow = S(row,:);
        temprow(lockedcol==1) = 1; % set locked columns to 1, so 
        % they are ignored by search for zeros below
...
for i = 1:n
    tempcol = Sinv(:,W(i));
    tempcol(newlockedrows==0) = ~tempcol(newlockedrows==0);
    %invert locked rows twice, thus leaving them where they started
    Sinv(:,W(i)) = tempcol;
    newlockedrows(L(i)) = 1;
end
end